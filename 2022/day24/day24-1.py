...
vertex = end_state
path = [vertex]
while vertex != starting_state:
    vertex = prev[vertex]
    path.append(vertex)
path.reverse()


# Print the steps involved:
print(f'One shortest path involves the following steps:')
pos = path[0][0]
for state in path:
    xy, _ = state
    dx, dy = xy[0] - pos[0], xy[1] - pos[1]
    if dx == 0:
        match dy:
            case -1:
                print(f' {pos}: Left (to {xy})')
            case 0:
                print(f' {pos}: Wait')
            case 1:
                print(f' {pos}: Right (to {xy})')
    elif dy == 0:
        match dx:
            case -1:
                print(f' {pos}: Up (to {xy})')
            case 1:
                print(f' {pos}: Down (to {xy})')
    pos = xy

print(f'The fastest way to reach the end position requires {dist[end_state]} minutes.')
