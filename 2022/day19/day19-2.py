...
def find_max_geodes(resources, bots, time_remaining, blueprint):
    ...
    # Optimizations:
    # Time cutoff after which we only build geode bots.
    time_only_geode_bot = 4
    ...
    if bots[0] > 0 and bots[0] < ore_max and time_remaining > time_only_geode_bot:  # Need at least 1 ore bot.
        bp = blueprint[0]
    ...
    if bots[0] > 0 and bots[1] < clay_max and time_remaining > time_only_geode_bot:  # Need at least 1 ore bot.
        bp = blueprint[1]
    ...
    if bots[0] > 0 and bots[1] > 0 and bots[2] < obs_max and time_remaining > time_only_geode_bot:  # Need at least 1 ore and 1 clay bot.
        bp = blueprint[2]
...
# Run through the first three blueprints and figure out how many geodes they produce after 32 minutes.
geode_product = 1
for bp_num, blueprint in enumerate(bp_list[:3]):
    g = find_max_geodes((0, 0, 0, 0), (1, 0, 0, 0), 32, blueprint)
    geode_product *= g
    print(f'Blueprint {bp_num+1}: Max {g} geodes.')

print(f'Product of all geode numbers: {geode_product}')
