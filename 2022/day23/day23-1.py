...
def do_round(m, direction_order):
    """Simulates a full round and returns the new map dict."""
    proposed_positions = {}
...
# Simulates the rounds.
NUM_STEPS = 10
for round in range(1, NUM_STEPS+1):
    map_dict = do_round(map_dict, direction_order)
    print(f'Step {round}:')
...
x_min = min([xy[0] for xy in map_dict])
x_max = max([xy[0] for xy in map_dict])
y_min = min([xy[1] for xy in map_dict])
y_max = max([xy[1] for xy in map_dict])
# Rectangle area minus the number of elves.
count = (x_max - x_min + 1) * (y_max - y_min + 1) - len(map_dict)
print(f'Number of ground tiles within the smallest rectangle: {count}')
