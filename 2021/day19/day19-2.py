...
# Find the largest (Manhattan) distance between two scanners.
max_distance = 0
for scanner_id in scanner_data:
    s_pos, _ = scanner_data[scanner_id]
    for n_id in scanner_data:
        n_pos, _ = scanner_data[n_id]
        distance = abs(s_pos[0] - n_pos[0]) + abs(s_pos[1] - n_pos[1]) + abs(s_pos[2] - n_pos[2])
        max_distance = max(distance, max_distance)

print(f'Largest distance between any two scanners: {max_distance}')
