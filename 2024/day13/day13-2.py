...
# Offset for the prize.
PRICE_OFFSET = 10_000_000_000_000
    ...
    for line in file.readlines():
        if line[:8] == 'Button A':
            ...
            claw_machine = {'a_x': int(a_x), 'a_y': int(a_y),
                            ...
                            'pr_x': int(prize_x) + PRICE_OFFSET, 'pr_y': int(prize_y) + PRICE_OFFSET}
...
for cm_num, cm in enumerate(claw_machines):
    ...
    M = np.array([[cm['a_x'], cm['b_x']],
                  [cm['a_y'], cm['b_y']]], np.int64)
    p = np.array([cm['pr_x'], cm['pr_y']], np.int64)
    try:
