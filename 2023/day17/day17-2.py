#!/usr/bin/python3
# Advent of Code 2023 - Day 17, Part 2
# Benedikt Otto
#
# The general idea is to do pathfinding with the Dijkstra algorithm, but with a
# modified graph that encodes the restriction of only being able to walk between
# 4 and 10 tiles in a straight line.
import math

# Open puzzle file and parse into dict(key=(x, y), val=symbol)
traffic_map = {}
#with open('example_17.txt') as file:
with open('../inputs/input_17.txt') as file:
    for row_num, row in enumerate(file.readlines()):
        row = row.strip()
        for col_num, char in enumerate(row):
            traffic_map.setdefault((row_num, col_num), int(char))

map_height = max([t[0] for t in traffic_map.keys()]) + 1
map_width = max([t[1] for t in traffic_map.keys()]) + 1


# Here each tuple (x, y, direction) is a vertex, and we assume at each vertex,
# that we are at the end of a straight line in that direction.
# So the vertex (10, 10, 'R') has the following fourteen neighbors:
#   (6, 10, 'U'), (5, 10, 'U'), (4, 10, 'U'), (3, 10, 'U'),
#   (2, 10, 'U'), (1, 10, 'U'), (0, 10, 'U'),
#   (14, 10, 'D'), (15, 10, 'D'), (16, 10, 'D'), (17, 10, 'D'),
#   (18, 10, 'D'), (19, 10, 'D'), (20, 10, 'D')

def get_neighbors(vertex_x, vertex_y, direction=''):
    """Create the list of all valid neighbors of the given vertex."""
    # Because we can't do a reverse, and we assume that we have done all the straight steps we needed,
    # we only have two possible directions left.
    match direction:
        case 'U':
            directions = ['L', 'R']
        case 'D':
            directions = ['L', 'R']
        case 'L':
            directions = ['U', 'D']
        case 'R':
            directions = ['U', 'D']
        case _:
            directions = ['U', 'D', 'L', 'R']

    neighbors = []
    for d in directions:
        for steps in range(4, 11):  # Neighbors for 4 to 10 steps.
            match d:
                case 'U':
                    new_x = vertex_x - steps
                    new_y = vertex_y
                    if 0 <= new_x < map_height and 0 <= new_y < map_width:
                        weight = 0
                        for k in range(1, steps + 1):
                            weight += traffic_map[(vertex_x - k, vertex_y)]
                        ne = ((new_x, new_y, 'U'), weight)
                        neighbors.append(ne)

                case 'D':
                    new_x = vertex_x + steps
                    new_y = vertex_y
                    if 0 <= new_x < map_height and 0 <= new_y < map_width:
                        weight = 0
                        for k in range(1, steps + 1):
                            weight += traffic_map[(vertex_x + k, vertex_y)]
                        ne = ((new_x, new_y, 'D'), weight)
                        neighbors.append(ne)

                case 'L':
                    new_x = vertex_x
                    new_y = vertex_y - steps
                    if 0 <= new_x < map_height and 0 <= new_y < map_width:
                        weight = 0
                        for k in range(1, steps + 1):
                            weight += traffic_map[(vertex_x, vertex_y - k)]
                        ne = ((new_x, new_y, 'L'), weight)
                        neighbors.append(ne)

                case 'R':
                    new_x = vertex_x
                    new_y = vertex_y + steps
                    if 0 <= new_x < map_height and 0 <= new_y < map_width:
                        weight = 0
                        for k in range(1, steps + 1):
                            weight += traffic_map[(vertex_x, vertex_y + k)]
                        ne = ((new_x, new_y, 'R'), weight)
                        neighbors.append(ne)

    return neighbors


# Define source vertex and create the graph.
source_vertex = (0, 0, '')
vertex_set = set()
vertex_set.add(source_vertex)

# Create set of all vertices.
for coord in traffic_map:
    (coord_x, coord_y) = coord
    for direction in ['U', 'D', 'L', 'R']:
        vertex = (coord_x, coord_y, direction)
        vertex_set.add(vertex)

# Create graph as dict(key=vertex, val=[neighbors])
traffic_graph = {}
for vertex in vertex_set:
    (vertex_x, vertex_y, direction) = vertex
    neighbors = []
    n_list = get_neighbors(vertex_x, vertex_y, direction)
    for n in n_list:
        neighbors.append(n)

    traffic_graph.setdefault(vertex, neighbors)

# Add destination vertex, its neighbors and add the destination as neighbor of all adjacent vertices.
destination_vertex = (map_height - 1, map_width - 1, '')
destination_neighbors = []
for direction in ['U', 'D', 'L', 'R']:
    vertex = (destination_vertex[0], destination_vertex[1], direction)
    destination_neighbors.append((vertex, 0))
    # Add edges to destination_vertex to all adjacent vertices.
    traffic_graph[vertex].append((destination_vertex, 0))
traffic_graph.setdefault(destination_vertex, destination_neighbors)


# DIJKSTRA ALGORITHM.
non_visited = []
dist = {}
prev = {}
for v in traffic_graph:
    # Prepare dicts.
    dist.setdefault(v, math.inf)
    prev.setdefault(v, None)
    non_visited.append(v)
dist[source_vertex] = 0

while len(non_visited) > 0:
    # Get vertex with minimum dist:
    minimum = math.inf
    for v in non_visited:
        if dist[v] < minimum:
            minimum = dist[v]
            min_vertex = v

    # Shortcut because we are only interested in the shortest path source -> destination.
    if min_vertex == destination_vertex:
        break  # Shortcut because we are only interested in the shortest path source -> destination.

    if minimum == math.inf:
        break  # Here the remaining nodes are unreachable.

    non_visited.remove(min_vertex)
    # Work through all neighbors of min_vertex.
    for n in traffic_graph[min_vertex]:
        (n_vertex, n_weight) = n
        # In this graph all edge values are 1.
        alt = dist[min_vertex] + n_weight
        if alt < dist[n_vertex]:
            dist[n_vertex] = alt
            prev[n_vertex] = min_vertex


# Get the shortest path by backtracking through prev and reversing the list.
path = []
node = destination_vertex
while node != source_vertex:
    node = prev[node]
    path.append(node)
path.reverse()

print(f'Shortest path to {destination_vertex}:')
print(path)
print(f'Length: {dist[destination_vertex]}')
