...
# Benedikt Otto
#
# Faster version with vertex = (x, y, direction).
#
# The general idea is to do pathfinding with the Dijkstra algorithm, but with a
# modified graph that encodes the restriction of only being able to walk a maximum
# of three tiles in a straight line.
import math
...
# So the vertex (5, 5, 'R') has the following six neighbors:
#   (4, 5, 'U'), (3, 5, 'U'), (2, 5, 'U')
#   (6, 5, 'D'), (7, 5, 'D'), (8, 5, 'D')

def get_neighbors(vertex_x, vertex_y, direction=''):
    """Create the list of all valid neighbors of the given vertex."""
    # Because we can't do a reverse, and we assume that we are at the end of a
    # straight segment, we only have two options left for edges.
    match direction:
        case 'U':
            directions = ['L', 'R']
        case 'D':
    ...
    neighbors = []
    for d in directions:
        for steps in range(1, 4):  # Neighbors for 1 to 3 steps.
            match d:
                case 'U':
